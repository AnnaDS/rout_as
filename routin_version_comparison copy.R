...
for( i in 1:length(numC)){
  ...
  #RN[which(RN[,grep(numC[i],colnames(RN))]!=""),grep(numC[i],colnames(RN))]=sapply(RN[which(RN[,grep(numC[i],colnames(RN))]!=""),grep(numC[i],colnames(RN))], as.numeric)
  
}
# print(head(RN))
# print("Attention")
# print(which(RN[,grep("raw_assigned_activities",colnames(RN))]==0))
if(nrow(RN)>0){
...
getCapacity <- function(initialV, stageV,  cmp='', initD, stageD, envI, envS){
 ...
 # print(initD, stageD)
  ...
  dv=data.frame(v=c("2.0.7.2", "2.1.0.2","2.1.1.0","2.2.0.0"), vF=c("2.0.7", "2.1.0","2.1.1", "2.2.0"))
...
#main function
getALLResF <- function(initialV, stageV,  cmp='vm', initD, stageD, init_env, stage_env, rp="", test=FALSE){
  RN=list()
  AFR=NULL
  cnv=c(initialV, stageV)
  dv=data.frame(v=c("2.0.7.2", "2.1.0.2","2.1.1.0", "2.2.0.0"), vF=c("2.0.7", "2.1.0","2.1.1", "2.2.0"))
 ...
 if(stage_env=="stage" && init_env=='prod' && test && is.null(t) ){
 paste(" No data")
   return(NULL)
 }
  if(stage_env=="stage" && init_env=='prod' && test && !is.null(t) && nrow(t)>0 && rp==''){
...
#    colnames(stageV)[grep("stage_routing_plan_name",colnames(stageV))]='routing_plan_name'
   ...
   dd=data.frame(routing_plan_name=initD$routing_plan_name, N_id_rgt=initD$N_id_rgt)
  ...
  print(length(dd$N_id_rgt))
  print(paste("R1",length(intersect(dd$N_id_rgt,R1$id_rgt))))
  print(paste("R2",length(intersect(dd$N_id_rgt,R2$id_rgt))))
  ...
  if(!all(dd$N_id_rgt==R2$id_rgt)) stop("The id's of stage assessment are not equal!!!")
   ...
   R1=ra11
   R2=ra22
   colnames(R1)=all_colls11
   colnames(R2)=all_colls22
   
   
   ra1c=NULL
   for(l in 1:nrow(R1)) {ra1c=c(ra1c,calc.v(R1[l,], idr, orig_vec))
     if(l==1) {plot(idr[1:5], t='l', lwd=3, col='red', main=cmp)}
     lines(as.numeric(R1[l,]), col='blue')}
   ra2c=NULL
   for(l in 1:nrow(R2)) {ra2c=c(ra2c,calc.v(R2[l,], idr, orig_vec))
                          lines(as.numeric(R2[l,]), col='darkgreen')}
   ...
   print(length(ids))
   print(nrow(dd))
   print(head(as.character(initD[,grep("_routing_plan_name",colnames(initD))])))
   D=data.frame(Routing_Plan=as.character(dd$routing_plan_name), id_rgt=dd$N_id_rgt,N1=rep(1,nrow(dd)), Val1=ra1c,  Val2=ra2c,  N2=rep(1,nrow(dd)))
   return(D)
 }
...
if(rp!=''){
  ...
  print(test)
  if(stage_env=="stage" & init_env=='prod' & test & !is.null(t) & nrow(t)>0){
    print('Test stage')
    t=t[!duplicated(t[,]),]
    print(head(t))
...
print(head(ra22))
  ...
  colnames(ra11)=c("id_rgt","version", "Assigned activities", "Work time", "Travel time", "Wait time", "Overtime","Providers","Losses %", "SLA_act", "SLA_time","Val")
  colnames(ra22)=c("id_rgt","version", "Assigned activities", "Work time", "Travel time", "Wait time", "Overtime","Providers","Losses %", "SLA_act", "SLA_time", "Val")
  for(p in 3:ncol(ra11)){
    ra11[,p]=round(as.numeric(as.character( ra11[,p])),3)
    ra22[,p]=round(as.numeric(as.character( ra22[,p])),3)
  }
...
print(nrow(t)==0)
if(nrow(inr)==0 | nrow(t)==0 ) return (NULL)
...
#print(tail(RA1))
  ...
  if(l==1) {plot(idr[1:5], t='l', lwd=3, col='red', main=RPf[i])}
...
R[which(R<0)]=0
    ...
    Rt=NULL
    for(h in 1:nrow(w)){
      #print(w[h,grep("N_id_rgt",colnames(w))])
...
Travel_as=(as.numeric(w[,grep("raw_travel_time_sum",colnames(w))]))/
 ...
 R=data.frame(id_rgt=as.numeric(w[,grep("N_id_rgt",colnames(w))]),AA=AA,  AAs=AAs,OvC=OvCt, TrC=TrCt, SlC=SlCt, RP=RP, AL=AL, WT=WT, RD=RD, RCD=RCD, Ovt=Ov, OvWt=OvWt,RV=R, SNA=SNA, RAA=RAA,RSAA=RSAA, RSLA=RSLA, TRAS=Travel_as, stringsAsFactors=F)
  if (length(AL[which(AL>1 | AL<0)])>0) stop(paste("Incorrect value N=", length(AL[which(AL>1 | AL<0)]), " ind = ",which(AL>1 | AL<0), 
             ...
             "N_id_rgt ", w[which(AL>1 | AL<0),grep("_id_rgt",colnames(w))]), call.=TRUE)
#print(replNAN(R))
  return (replNAN(R))
}
...
getRPdata <- function(w, total=NULL, v=TRUE){
 ...
 # if(length(fn)>0)  w=w[-fn,] # remove the routs that don't provide any results
  ...
  AA=total- as.numeric(w[,grep("raw_not_assigned_activities2",colnames(w))])- as.numeric(w[,grep("raw_rejected_activities",colnames(w))])
