...
#                            print(initD$N_id_rgt[l])
#                            print(ra11[l,])
#                            print(paste(as.character(dd$routing_plan_name[l]), ra1c[length(ra1c)]))
    # print(paste("IDR", idr))                      
...
if (length(AL[which(AL>1 | AL<0)])>0){
 k=unique(which(AL>1 | AL<0))
  print(paste("Incorrect value N=", as.numeric(AL[k]), " ind = ",k, 
             "routing plan name ", as.character(w[,grep("routing_plan_name",colnames(w))][k]),
             "N_id_rgt ", as.numeric(w[k,grep("_id_rgt",colnames(w))]))#, call.=TRUE
  )}
#print(replNAN(R))
  return (replNAN(R))
}
